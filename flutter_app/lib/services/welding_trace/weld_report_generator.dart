...
///   3. Curve summary (duration, max pressure, average pressure, sample count)
///   4. Pressure × time chart (vector line rendering; "No data" if < 2 points)
///   5. Digital signature block (SHA-256 fingerprint)
///   6. QR code encoding the weld signature for mobile scanner access
///   7. Footer — "Generated by WeldTrace"
///
/// PDF generation is designed to be non-fatal:
///   • Chart rendering is wrapped in a try/catch — if it fails the section
///     shows a "Chart unavailable" message instead.
///   • An empty or single-sample curve produces a valid report without a chart.
///
/// Usage:
/// ```dart
...
class WeldReportGenerator {
  ...
  /// "No data recorded" message.  PDF generation never throws — exceptions
  /// from chart rendering are caught and replaced with a fallback message.
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.SizedBox(height: 6),
          _signatureRow(weldSignature, rowAltColour, accentColour),
          pw.SizedBox(height: 16),
        ],
      ),
    );
  ...
  /// Renders the pressure × time chart.
  ///
  /// If [curve] has fewer than 2 points, shows a "No data recorded" label.
  /// Chart painting is wrapped in try/catch — if the painter throws for any
  /// reason, a "Chart unavailable" fallback is shown instead of propagating.
  ...
  /// Renders the signature text block and QR code side by side.
  ///
  /// The QR code encodes the weld signature (64-char SHA-256 hex) so that
  /// field inspectors can verify the weld with a mobile scanner.
...
/// Renders a QR code for [data] directly onto a PDF canvas.
///
/// Uses [QrCode] (pure-Dart `qr` package) to generate the module matrix, then
/// draws each dark module as a filled rectangle via [PdfGraphics.drawRect].
///
/// The quiet zone (4-module white border) is included implicitly by the
/// [QrCode] library.
      ...
      for (int row = 0; row < moduleCount; row++) {
        for (int col = 0; col < moduleCount; col++) {
          if (qrCode.isDark(row, col)) {
            ...
            // QR rows: top-down, so row 0 maps to top of canvas.
